...
//===============================================================check driver ban
async function checkDriverban (){
  try {
    if(dvrCnic){

  axios.get(`${BASE_URL}/web/ban/checkDriverban/${dvrCnic}`).then(
    response=>{
        const result = response.data[0]
        console.log(result)
      if (result){
      if(result.banStatus =='ban'){
        if(result.banArea == 'sector'){
            if(result.banoffice == currentUser.sector){
                Alert.alert('Driver Ban Alert ⚠️',`Mr. ${result.driverName} S/O ${result.fatherName}\n CNIC # ${result.cnic} \n  is Banned in Sector : ${currentUser.sector}  \n due to ${result.reasonTitle}  \n From:  ${result.startDate.split("-").reverse().join("-")} \n To: ${result.endDate.split("-").reverse().join("-")}` )
            }
        }
        else if (result.banArea == 'zone'){
          console.log(result.banArea,currentUser.zone)
            if(result.banoffice == currentUser.zone){
              Alert.alert('Driver Ban Alert ⚠️',`Mr. ${result.driverName} S/O ${result.fatherName} \n CNIC # ${result.cnic} is banned in \n Zone : ${currentUser.zone}   \n due to ${result.reasonTitle} \n From: ${result.startDate.split("-").reverse().join("-")} \n To: ${result.endDate.split("-").reverse().join("-")}` )
            }
        }
        else if (result.banArea == 'region'){
            if(result.banoffice == currentUser.region){
